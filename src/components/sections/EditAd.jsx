...
export default function EditAdPage({ adDetails }) {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      const res = await fetch(
        process.env.NEXT_PUBLIC_BASE_URL + `/api/ads/edit/${adDetails.id}`,
        {
          method: "POST",
          headers: {
  ...
  return (
    <>
      ...
      <div className="edit-form p-7">
        <div className="py-3">
          <h3 className="font-bold text-xl">Edit {"Ad Name here"}</h3>
        </div>
        <hr />
        {/* form */}
        <form onSubmit={formik.handleSubmit} className="mt-5">
          <div className="form grid grid-cols-12 gap-5">
            ...
            <div className="col-span-12 lg:col-span-6">
              <label htmlFor="name" className="font-regular text-gray-500">
                Saler Contact No <span className="text-xs text-red-500">*</span>
              </label>
              <input
                id="contact_no"
                name="contact_no"
