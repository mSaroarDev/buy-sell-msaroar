...
export default function AdsPaginationAllAds({ totalAds }) {
  ...
  return (
    <>
      <div className="join">
        <Link href={`${process.env.NEXT_PUBLIC_BASE_URL}/explore-ads?page=1`} className={`join-item btn`}>
          «
        </Link>
        {Math.ceil(totalAds / 10) <= 1 ? (
          <Link
            href={`${process.env.NEXT_PUBLIC_BASE_URL}/explore-ads?page=1`}
        ...
        <Link
          href={`/explore-ads?page=${Math.ceil(totalAds / 10)}`}
